...
// WKT Marshal/Unmarshal cycle, asserting layout and ordinates survive. The
// MULTIPOINT cases guard the previously-dropped Z/M decoder bug.
func TestZMConstructorRoundTrip(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			s, err := Marshal(tc.src)
			require.NoError(t, err)
			got, err := Unmarshal(s)
			require.NoError(t, err)
			assert.Equal(t, tc.layout, got.Layout(), "layout (wkt=%q)", s)
			fc := got.(interface{ AppendFlatCoords([]float64) []float64 })
			assert.Equal(t, tc.flat, fc.AppendFlatCoords(nil), "ordinates (wkt=%q)", s)
		})
	}
}
